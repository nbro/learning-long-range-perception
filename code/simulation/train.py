...
def train():
    ...
    parser.add_argument('-n', '--name', type=str, help='name of the Model weights',
    ...
    parser.add_argument('-sp', '--split-percentage', type=float, help='train/test split percentage (0:100)',
                        default=50.0)
    ...
    gen = generator(split_percentage=split_percentage, filename=filename, batch_size=batch_size, is_testset=False,
                    augment=True)

    val_x, val_y, _ = next(generator(filename=filename, is_testset=True, testset_index=-1))

    history = cnn.fit_generator(generator=gen, steps_per_epoch=steps, epochs=n_epochs, validation_data=(val_x, val_y),
                                callbacks=[
                                    keras.callbacks.ModelCheckpoint(name + '/weights.{epoch:02d}-{val_loss:.4f}.h5',
                                                                    save_best_only=True,
